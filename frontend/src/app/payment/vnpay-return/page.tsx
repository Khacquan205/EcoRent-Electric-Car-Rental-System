  ...
  return (
    <div className="mx-auto max-w-md px-4 py-16">
      <div className="rounded-2xl border border-slate-200 bg-white p-8 shadow-sm text-center">
        ...
        {success && (result?.transactionId || result?.amount != null || result?.payDate) && (
          <div className="mt-6 rounded-xl border border-slate-200 bg-slate-50 p-4 text-left">
            ...
            {result?.orderId && (
              <p className="text-sm text-slate-700">
                <span className="text-slate-500">Mã đơn:</span> {result.orderId}
              </p>
            )}
            {result?.transactionId && (
              <p className="mt-1 text-sm text-slate-700">
                <span className="text-slate-500">Mã GD:</span> {result.transactionId}
              </p>
            )}
            {result?.amount != null && (
              <p className="mt-1 text-sm text-slate-700">
        ...
        <div className="mt-8 flex flex-col gap-3">
          ...
          <Link href="/owner/post/new" className="text-sm text-primary hover:underline">
            Đăng tin xe
          </Link>
        </div>
      </div>
